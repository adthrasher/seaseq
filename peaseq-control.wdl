...
workflow peaseq {
    ...
    if (defined(sample_sraid)) {
        ...
        scatter (eachsra in s_sraid) {
            ...
            File R1end = select_first([
                fastqdump.R1end,
                string_sra[0],
            ])
            File R2end = select_first([
                fastqdump.R2end,
                string_sra[0],
            ])
    ...
    if (defined(control_sraid)) {
        ...
        scatter (eachsra in c_sraid) {
            call sra.fastqdump as c_fastqdump { input:
                sra_id = eachsra,
                cloud = false,
            }
            File c_R1end = select_first([
                c_fastqdump.R1end,
                c_sra[0],
            ])
            File c_R2end = select_first([
                c_fastqdump.R2end,
                c_sra[0],
            ])
        }  # end scatter each sra
    ...
    call samtools.faidx as samtools_faidx { input: reference = reference }
    # effective genome size for FASTA
    call util.effective_genome_size as egs { input: reference = reference }
    ...
    if (defined(sample_R1_fastq)) {
        ...
        Array[File] s_R1_fastq = select_first([
            sample_R1_fastq,
            string_fastq,
        ])
        Array[File] sample_R1_fastqfile = s_R1_fastq
        Array[File] s_R2_fastq = select_first([
            sample_R2_fastq,
            string_fastq,
        ])
        ...
        Array[File] sample_R2_fastqfiles = select_first([
            R2_sorted.allfiles,
            sample_R2_fastqfile,
        ])
    }
    ...
    if (defined(control_R1_fastq)) {
        Array[String] c_string_fastq = [
            "1",
        ]  #buffer to allow for fastq optionality
        Array[File] c_R1_fastq = select_first([
            control_R1_fastq,
            c_string_fastq,
        ])
        Array[File] control_R1_fastqfile = c_R1_fastq
        Array[File] c_R2_fastq = select_first([
            control_R2_fastq,
            c_string_fastq,
        ])
        ...
        Array[File] control_R1_fastqfiles = select_first([
            c_R1_sorted.allfiles,
            control_R1_fastqfile,
        ])
        Array[File] control_R2_fastqfiles = select_first([
            c_R2_sorted.allfiles,
            control_R2_fastqfile,
        ])
    }

    # collate all fastqfiles
    Array[File] original_sample_R1 = flatten(select_all([
        sample_R1_srafile,
        sample_R1_fastqfiles,
    ]))
    Array[File] original_sample_R2 = flatten(select_all([
        sample_R2_srafile,
        sample_R2_fastqfiles,
    ]))
    Array[File] original_all_sample_fastqfiles = flatten(select_all([
        sample_R1_srafile,
        sample_R1_fastqfiles,
        sample_R2_srafile,
        sample_R2_fastqfiles,
    ]))
    Array[File] original_control_R1 = flatten(select_all([
        control_R1_srafile,
        control_R1_fastqfiles,
    ]))
    Array[File] original_control_R2 = flatten(select_all([
        control_R2_srafile,
        control_R2_fastqfiles,
    ]))
    Array[File] original_all_control_fastqfiles = flatten(select_all([
        control_R1_srafile,
        control_R1_fastqfiles,
        control_R2_srafile,
        control_R2_fastqfiles,
    ...
    Boolean multi_control_fastqpair = if length(original_control_fastqfiles) > 1 then true
        else false
    Boolean one_control_fastqpair = if length(original_control_fastqfiles) == 1 then true
        else false

    if (defined(spikein_bowtie_index) || defined(spikein_reference)) {
        scatter (eachfastq in original_all_sample_fastqfiles) {
            call fastqc.fastqc as spikein_s_indv_fastqc { input:
                inputfile = eachfastq,
                default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                    basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/SpikeIn/FastQC"
                    else "SAMPLE/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/SpikeIn/FastQC",
            }
        }
        scatter (eachfastq in original_all_control_fastqfiles) {
            call fastqc.fastqc as spikein_c_indv_fastqc { input:
                inputfile = eachfastq,
                default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                    + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "")
                    + "/SpikeIn/FastQC" else "CONTROL/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/SpikeIn/FastQC",
            }
        }
        scatter (fastqpair in original_sample_fastqfiles) {
            call util.basicfastqstats as spikein_s_indv_R1_bfs { input:
            ...
            call bowtie.spikein_PE as spikein_s_indv_map { input:
                fastqfile = fastqpair.left,
        ...
        scatter (fastqpair in original_control_fastqfiles) {
            call util.basicfastqstats as spikein_c_indv_R1_bfs { input:
                fastqfile = fastqpair.left,
                default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
            ...
            call bowtie.spikein_PE as spikein_c_indv_map { input:
                fastqfile = fastqpair.left,
                fastqfile_R2 = fastqpair.right,
        ...
        Array[File] spikein_sample_R1 = select_first([
            spikein_s_R1_sorted.allfiles,
            spikein_s_indv_map.unaligned_1,
        ])
        Array[File] spikein_sample_R2 = select_first([
            spikein_s_R2_sorted.allfiles,
            spikein_s_indv_map.unaligned_2,
        ])
        Array[File] spikein_control_R1 = select_first([
            spikein_c_R1_sorted.allfiles,
            spikein_c_indv_map.unaligned_1,
        ])
        Array[File] spikein_control_R2 = select_first([
            spikein_c_R2_sorted.allfiles,
    ...
    Array[File] all_sample_fastqfiles = select_first([
        spikein_all_sample_fastqfiles,
        original_all_sample_fastqfiles,
    ])
    Array[Pair[File, File]] sample_fastqfiles = select_first([
        spikein_sample_fastqfiles,
        original_sample_fastqfiles,
    ])
    Array[File] all_control_fastqfiles = select_first([
        spikein_all_control_fastqfiles,
        original_all_control_fastqfiles,
    ])
    Array[Pair[File, File]] control_fastqfiles = select_first([
        spikein_control_fastqfiles,
        original_control_fastqfiles,
    ])
    ...
    scatter (eachfastq in all_sample_fastqfiles) {
        call fastqc.fastqc as s_indv_fastqc { input:
            inputfile = eachfastq,
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/FastQC"
                else "SAMPLE/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/FastQC",
        }

        call util.basicfastqstats as indv_s_bfs { input:
            fastqfile = eachfastq,
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats"
                else "SAMPLE/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats",
        }

        call mapping.mapping as indv_s_mapping { input:
            fastqfile = eachfastq,
            index_files = actual_bowtie_index,
            metricsfile = indv_s_bfs.metrics_out,
            blacklist = blacklist,
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/BAM_files"
                else "SAMPLE/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/BAM_files",
        }

        call fastqc.fastqc as indv_s_bamfqc { input:
        ...
        call runspp.runspp as indv_s_runspp { input: bamfile = select_first([
            indv_s_mapping.bklist_bam,
            indv_s_mapping.sorted_bam,
        ]) }

        call bedtools.bamtobed as indv_s_bamtobed { input: bamfile = select_first([
            indv_s_mapping.bklist_bam,
            indv_s_mapping.sorted_bam,
        ]) }

        call util.evalstats as indv_s_summarystats { input:
            ...
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/single-end_mode/QC/SummaryStats"
                else "SAMPLE/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/single-end_mode/QC/SummaryStats",
        }
    ...
    call util.mergehtml as SE_s_mergehtml { input:
    ...
    call samtools.mergebam as SE_s_mergebam { input:
        bamfiles = indv_s_mapping.sorted_bam,
        metricsfiles = indv_s_bfs.metrics_out,
        default_location = if multi_fastqpair then "SAMPLE/" + "AllCases_" + length(
            sample_fastqfiles) + "fastqpairs/single-end_mode/BAM_files" else "SAMPLE/" + sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/BAM_files",
        outputfile = "AllCases_" + length(all_sample_fastqfiles) + "fastqs_SE.sorted.bam",
    }

    call fastqc.fastqc as SE_s_mergebamfqc { input:
        inputfile = SE_s_mergebam.mergebam,
        default_location = if multi_fastqpair then "SAMPLE/" + "AllCases_" + length(
            sample_fastqfiles) + "fastqpairs/single-end_mode/QC/FastQC" else "SAMPLE/" + sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/QC/FastQC",
    }

    call samtools.indexstats as SE_s_mergeindexstats { input:
    ...
    if (defined(blacklist)) {
        ...
        call bedtools.intersect as SE_s_merge_rmblklist { input:
            fileA = SE_s_mergebam.mergebam,
            fileB = blacklist_file,
            default_location = if multi_fastqpair then "SAMPLE/" + "AllCases_" + length(
                sample_fastqfiles) + "fastqpairs/single-end_mode/BAM_files" else "SAMPLE/"
                + sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/single-end_mode/BAM_files",
            nooverlap = true,
        }
        call samtools.indexstats as SE_s_merge_bklist { input:
    ...
    File SE_s_mergebam_afterbklist = select_first([
        SE_s_merge_rmblklist.intersect_out,
        SE_s_mergebam.mergebam,
    ])

    call samtools.markdup as SE_s_merge_markdup { input:
    ...
    call samtools.indexstats as SE_s_merge_mkdup { input:
    ...
    scatter (eachfastq in all_control_fastqfiles) {
        call fastqc.fastqc as c_indv_fastqc { input:
            inputfile = eachfastq,
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/FastQC"
                else "CONTROL/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/FastQC",
        }

        call util.basicfastqstats as indv_c_bfs { input:
            fastqfile = eachfastq,
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats"
                else "CONTROL/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats",
        }

        call mapping.mapping as indv_c_mapping { input:
            fastqfile = eachfastq,
            index_files = actual_bowtie_index,
            metricsfile = indv_c_bfs.metrics_out,
            blacklist = blacklist,
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/BAM_files"
                else "CONTROL/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/BAM_files",
        }

        call fastqc.fastqc as indv_c_bamfqc { input:
        ...
        call runspp.runspp as indv_c_runspp { input: bamfile = select_first([
            indv_c_mapping.bklist_bam,
            indv_c_mapping.sorted_bam,
        ]) }

        call bedtools.bamtobed as indv_c_bamtobed { input: bamfile = select_first([
            indv_c_mapping.bklist_bam,
            indv_c_mapping.sorted_bam,
        ]) }

        call util.evalstats as indv_c_summarystats { input:
            ...
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/single-end_mode/QC/SummaryStats"
                else "CONTROL/" + sub(basename(eachfastq), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/single-end_mode/QC/SummaryStats",
        }
    ...
    call util.mergehtml as SE_c_mergehtml { input:
    ...
    call samtools.mergebam as SE_c_mergebam { input:
        ...
        default_location = if multi_control_fastqpair then "CONTROL/" + "AllControls_" + length(
            control_fastqfiles) + "fastqpairs/single-end_mode/BAM_files" else "CONTROL/" + sub(
            basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/BAM_files",
        outputfile = "AllControls_" + length(all_control_fastqfiles) + "fastqs_SE.sorted.bam",
    }

    call fastqc.fastqc as SE_c_mergebamfqc { input:
        inputfile = SE_c_mergebam.mergebam,
        default_location = if multi_control_fastqpair then "CONTROL/" + "AllControls_" + length(
            control_fastqfiles) + "fastqpairs/single-end_mode/QC/FastQC" else "CONTROL/" + sub(
            basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/QC/FastQC",
    }

    call samtools.indexstats as SE_c_mergeindexstats { input:
    ...
    if (defined(blacklist)) {
        ...
        File c_blacklist_file = select_first([
            blacklist,
            c_string_blacklist,
        ])
        call bedtools.intersect as SE_c_merge_rmblklist { input:
            fileA = SE_c_mergebam.mergebam,
            fileB = c_blacklist_file,
            default_location = if multi_control_fastqpair then "CONTROL/" + "AllControls_"
                + length(control_fastqfiles) + "fastqpairs/single-end_mode/BAM_files" else "CONTROL/"
                + sub(basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/single-end_mode/BAM_files",
            nooverlap = true,
        }
        call samtools.indexstats as SE_c_merge_bklist { input:
    ...
    File SE_c_mergebam_afterbklist = select_first([
        SE_c_merge_rmblklist.intersect_out,
        SE_c_mergebam.mergebam,
    ])

    call samtools.markdup as SE_c_merge_markdup { input:
    ...
    call samtools.indexstats as SE_c_merge_mkdup { input:
    ...
    if (multi_fastqpair) {
        scatter (fastqpair in sample_fastqfiles) {
            ...
            call util.basicfastqstats as s_indv_R1_bfs { input:
                fastqfile = fastqpair.left,
                default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                    basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats"
                    else "SAMPLE/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/SummaryStats",
            }

            call mapping.mapping as s_indv_PE_mapping { input:
                fastqfile = fastqpair.left,
                fastqfile_R2 = fastqpair.right,
                ...
                paired_end = true,
                default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                    basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/BAM_files"
                    else "SAMPLE/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/BAM_files",
            }

            call fastqc.fastqc as s_indv_PE_bamfqc { input:
                inputfile = s_indv_PE_mapping.sorted_bam,
                default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                    basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/FastQC"
                    else "SAMPLE/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/FastQC",
            }

            call runspp.runspp as s_indv_PE_runspp { input: bamfile = select_first([
                s_indv_PE_mapping.bklist_bam,
                s_indv_PE_mapping.sorted_bam,
            ]) }

            call peaseq_util.pe_bamtobed as s_indv_PE_bamtobed { input: bamfile = select_first(
            [
                s_indv_PE_mapping.bklist_bam,
                s_indv_PE_mapping.sorted_bam,
            ]) }

            call util.evalstats as s_indv_PE_summarystats { input:
                fastq_type = "PEAseq Sample FASTQ",
        ...
        call util.mergehtml as s_PE_mergehtml { input:
        ...
        call peaseq_util.pe_mergehtml as s_final_mergehtml { input:
        ...
        call samtools.mergebam as s_PE_mergebam { input:
            bamfiles = s_indv_PE_mapping.as_sortedbam,
            metricsfiles = indv_s_bfs.metrics_out,
            paired_end = true,
            default_location = "SAMPLE/" + "AllCases_" + length(s_indv_PE_mapping.sorted_bam
                ) + "fastqpairs/BAM_files",
        ...
        call fastqc.fastqc as s_PE_mergebamfqc { input:
            inputfile = s_PE_mergebam.mergebam,
            default_location = "SAMPLE/" + sub(basename(s_PE_mergebam.mergebam), ".sorted.b.*$",
                "") + "/QC/FastQC",
        }

        call samtools.indexstats as s_PE_mergeindexstats { input:
        ...
        if (defined(blacklist)) {
            ...
            File s_blacklist_pe = select_first([
                blacklist,
                s_string_pe_blacklist,
            ])
            String s_string_pe_fixmate = ""  #buffer to allow for blacklist optionality
            File s_fixmate_pe = select_first([
                s_PE_mergebam.fixmatemergebam,
                s_string_pe_fixmate,
            ])
            call bedtools.pairtobed as s_PE_merge_rmblklist { input:
                fileA = s_fixmate_pe,
                fileB = s_blacklist_pe,
                default_location = "SAMPLE/" + sub(basename(s_PE_mergebam.mergebam), ".sorted.b.*$",
                    "") + "/BAM_files",
            }
            call samtools.indexstats as s_PE_merge_bklist { input:
        ...
        File s_PE_mergebam_afterbklist = select_first([
            s_PE_merge_rmblklist.pairtobed_out,
            s_PE_mergebam.mergebam,
        ])

        call samtools.markdup as s_PE_merge_markdup { input:
            bamfile = s_PE_mergebam_afterbklist,
            default_location = "SAMPLE/" + sub(basename(s_PE_mergebam.mergebam), ".sorted.b.*$",
                "") + "/BAM_files",
        }

        call samtools.indexstats as s_PE_merge_mkdup { input:
    ...
    if (multi_control_fastqpair) {
        scatter (fastqpair in control_fastqfiles) {
            ...
            call util.basicfastqstats as c_indv_R1_bfs { input:
                fastqfile = fastqpair.left,
                default_location = if multi_fastqpair then "CONTROL/individual_fastqs/" + sub(
                    basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats"
                    else "CONTROL/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/SummaryStats",
            }

            call mapping.mapping as c_indv_PE_mapping { input:
                fastqfile = fastqpair.left,
                fastqfile_R2 = fastqpair.right,
                ...
                default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                    + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/BAM_files" else "CONTROL/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/BAM_files",
            }

            call fastqc.fastqc as c_indv_PE_bamfqc { input:
                inputfile = c_indv_PE_mapping.sorted_bam,
                default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                    + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/FastQC" else "CONTROL/" + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/FastQC",
            }

            call runspp.runspp as c_indv_PE_runspp { input: bamfile = select_first([
                c_indv_PE_mapping.bklist_bam,
                c_indv_PE_mapping.sorted_bam,
            ]) }

            call peaseq_util.pe_bamtobed as c_indv_PE_bamtobed { input: bamfile = select_first(
            [
                c_indv_PE_mapping.bklist_bam,
                c_indv_PE_mapping.sorted_bam,
            ]) }

            call util.evalstats as c_indv_PE_summarystats { input:
                fastq_type = "PEAseq Control FASTQ",
                ...
                default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                    + sub(basename(fastqpair.left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                    "") + "/QC/SummaryStats" else "CONTROL/" + sub(basename(fastqpair.left
                    ), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats",
            }
        }  # end scatter (for each sample fastq)
        ...
        # merge bam files and perform fasTQC if more than one is provided
        call util.mergehtml as c_PE_mergehtml { input:
        ...
        call peaseq_util.pe_mergehtml as c_final_mergehtml { input:
            fastq_type = "PEAseq Control FASTQs",
        ...
        call samtools.mergebam as c_PE_mergebam { input:
            bamfiles = c_indv_PE_mapping.as_sortedbam,
            metricsfiles = indv_c_bfs.metrics_out,
            paired_end = true,
            default_location = "CONTROL/" + "AllControls_" + length(c_indv_PE_mapping.sorted_bam
                ) + "fastqpairs/BAM_files",
        ...
        call fastqc.fastqc as c_PE_mergebamfqc { input:
            inputfile = c_PE_mergebam.mergebam,
            default_location = "CONTROL/" + sub(basename(c_PE_mergebam.mergebam), ".sorted.b.*$",
                "") + "/QC/FastQC",
        }

        call samtools.indexstats as c_PE_mergeindexstats { input:
        ...
        if (defined(blacklist)) {
            ...
            File c_blacklist_pe = select_first([
                blacklist,
                c_string_pe_blacklist,
            ])
            String c_string_pe_fixmate = ""  #buffer to allow for blacklist optionality
            File c_fixmate_pe = select_first([
                c_PE_mergebam.fixmatemergebam,
                c_string_pe_fixmate,
            ])
            call bedtools.pairtobed as c_PE_merge_rmblklist { input:
                fileA = c_fixmate_pe,
                fileB = c_blacklist_pe,
                default_location = "CONTROL/" + sub(basename(c_PE_mergebam.mergebam), ".sorted.b.*$",
                    "") + "/BAM_files",
            }
            call samtools.indexstats as c_PE_merge_bklist { input:
        ...
        File c_PE_mergebam_afterbklist = select_first([
            c_PE_merge_rmblklist.pairtobed_out,
            c_PE_mergebam.mergebam,
        ])

        call samtools.markdup as c_PE_merge_markdup { input:
            bamfile = c_PE_mergebam_afterbklist,
            default_location = "CONTROL/" + sub(basename(c_PE_mergebam.mergebam), ".sorted.b.*$",
                "") + "/BAM_files",
        }

        call samtools.indexstats as c_PE_merge_mkdup { input:
    ...
    if (one_fastqpair) {
        ...
        call util.basicfastqstats as s_R1_bfs { input:
            fastqfile = sample_fastqfiles[0].left,
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats" else "SAMPLE/" + sub(basename(sample_fastqfiles[0
                ].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats",

        }

        call mapping.mapping as s_uno_PE_mapping { input:
            fastqfile = sample_fastqfiles[0].left,
            ...
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/BAM_files" else "SAMPLE/" + sub(basename(sample_fastqfiles[0].left
                ), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/BAM_files",
        }

        call fastqc.fastqc as s_uno_PE_bamfqc { input:
            inputfile = s_uno_PE_mapping.sorted_bam,
            default_location = if multi_fastqpair then "SAMPLE/individual_fastqs/" + sub(
                basename(s_uno_PE_mapping.sorted_bam), ".sorted.bam", "") + "/QC/FastQC"
                else "SAMPLE/" + sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/FastQC",
        }

        call peaseq_util.pe_bamtobed as s_uno_PE_bamtobed { input: bamfile = select_first(
        [
            s_uno_PE_mapping.bklist_bam,
            s_uno_PE_mapping.sorted_bam,
        ]) }
    ...
    if (one_control_fastqpair) {
        ...
        call util.basicfastqstats as c_R1_bfs { input:
            fastqfile = control_fastqfiles[0].left,
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats" else "CONTROL/" + sub(basename(control_fastqfiles[
                0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/QC/SummaryStats",
        }

        call mapping.mapping as c_uno_PE_mapping { input:
            fastqfile = control_fastqfiles[0].left,
            ...
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/BAM_files" else "CONTROL/" + sub(basename(control_fastqfiles[0].left
                ), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "/BAM_files",
        }

        call fastqc.fastqc as c_uno_PE_bamfqc { input:
            inputfile = c_uno_PE_mapping.sorted_bam,
            default_location = if multi_control_fastqpair then "CONTROL/individual_fastqs/"
                + sub(basename(c_uno_PE_mapping.sorted_bam), ".sorted.bam", "") + "/QC/FastQC"
                else "CONTROL/" + sub(basename(c_uno_PE_mapping.sorted_bam), ".sorted.bam",
                "") + "/QC/FastQC",
        }

        call peaseq_util.pe_bamtobed as c_uno_PE_bamtobed { input: bamfile = select_first(
        [
            c_uno_PE_mapping.bklist_bam,
            c_uno_PE_mapping.sorted_bam,
        ]) }
    }  # end if length(fastqfiles) == 1: one_fastqpair
    ...
    call macs.macs as SE_macs { input:
        bamfile = SE_s_mergebam_afterbklist,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-p9_kd-auto'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-auto"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/PEAKS/NARROW_peaks" + "/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-auto",
        coverage_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-p9_kd-auto'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-auto"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks" + "/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-auto",
    }

    call util.addreadme as SE_addreadme { input: default_location = if defined(
        results_name) then "~{results_name + '/single-end_mode/PEAKS'}" else if multi_fastqpair
        then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS"
        else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
        "") + "+control/single-end_mode/PEAKS" }

    call macs.macs as SE_all { input:
        bamfile = SE_s_mergebam_afterbklist,
        control = SE_c_mergebam_afterbklist,
        pvalue = "1e-9",
        keep_dup = "all",
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-p9_kd-all'}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-all"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/PEAKS/NARROW_peaks" + "/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-all",
        coverage_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-p9_kd-all'}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-all"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks" + "/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "+control-p9_kd-all",
    ...
    call macs.macs as SE_nomodel { input:
        bamfile = SE_s_mergebam_afterbklist,
        control = SE_c_mergebam_afterbklist,
        nomodel = true,
        egs = egs.genomesize,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-nm'}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-nm" else sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "+control/single-end_mode/PEAKS/NARROW_peaks" + "/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "+control-nm",
        coverage_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks'
            + '/' + basename(SE_s_mergebam_afterbklist, '.bam') + '+control-nm'}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/NARROW_peaks"
            + "/" + basename(SE_s_mergebam_afterbklist, ".bam") + "+control-nm" else sub(
    ...
    call bamtogff.bamtogff as SE_bamtogff { input:
        gtffile = gtf,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/BAM_Density'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/BAM_Density"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/BAM_Density",
    }

    call bedtools.bamtobed as SE_s_forsicerbed { input: bamfile = SE_s_merge_markdup.mkdupbam
        }

    call bedtools.bamtobed as SE_c_forsicerbed { input: bamfile = SE_c_merge_markdup.mkdupbam
        }

    call sicer.sicer as SE_sicer { input:
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS/BROAD_peaks'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS/BROAD_peaks"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/PEAKS/BROAD_peaks",
        coverage_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/BROAD_peaks'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/BROAD_peaks"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/COVERAGE_files/BROAD_peaks",
    }

    call rose.rose as SE_rose { input:
        gtffile = gtf,
        bamfile = SE_s_merge_markdup.mkdupbam,
        bamindex = SE_s_merge_mkdup.indexbam,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS/STITCHED_peaks'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS/STITCHED_peaks"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/PEAKS/STITCHED_peaks",
    }

    call runspp.runspp as SE_runspp { input:
        bamfile = SE_s_mergebam_afterbklist,
        control = SE_c_mergebam_afterbklist,
    }

    call runspp.runspp as SE_s_runspp { input: bamfile = SE_s_mergebam_afterbklist }

    call runspp.runspp as SE_c_runspp { input: bamfile = SE_c_mergebam_afterbklist }

    String SE_string_ctrlwig = ""
    call viz.visualization as SE_c_visualization { input:
        wigfile = select_first([
            SE_macs.ctrlwigfile,
            SE_string_ctrlwig,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks/'
            + sub(basename(SE_macs.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(SE_s_mergebam_afterbklist), ".sorted.b.*$", "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks/"
            + sub(basename(SE_macs.peakbedfile), "_peaks.bed", "") + "/control",
    }

    call viz.visualization as SE_c_vizall { input:
        wigfile = select_first([
            SE_all.ctrlwigfile,
            SE_string_ctrlwig,
        ]),
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks/'
            + sub(basename(SE_all.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(SE_s_mergebam_afterbklist), ".sorted.b.*$", "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks/"
            + sub(basename(SE_all.peakbedfile), "_peaks.bed", "") + "/control",
    }
    call viz.visualization as SE_c_viznomodel { input:
        wigfile = select_first([
            SE_nomodel.ctrlwigfile,
            SE_string_ctrlwig,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        control = true,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks/'
            + sub(basename(SE_nomodel.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(SE_s_mergebam_afterbklist), ".sorted.b.*$", "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks/"
            + sub(basename(SE_nomodel.peakbedfile), "_peaks.bed", "") + "/control",
    }

    call util.peaksanno as SE_peaksanno { input:
        gtffile = gtf,
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS_Annotation/NARROW_peaks'
            + '/' + sub(basename(SE_macs.peakbedfile), '_peaks.bed', '')}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS_Annotation/NARROW_peaks"
            + "/" + sub(basename(SE_macs.peakbedfile), "_peaks.bed", "") else sub(basename(
            sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "+control/single-end_mode/PEAKS_Annotation/NARROW_peaks"
            + "/" + sub(basename(SE_macs.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as SE_all_peaksanno { input:
        gtffile = gtf,
        bedfile = SE_all.peakbedfile,
        chromsizes = samtools_faidx.chromsizes,
        summitfile = SE_all.summitsfile,
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS_Annotation/NARROW_peaks'
            + '/' + sub(basename(SE_all.peakbedfile), '_peaks.bed', '')}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS_Annotation/NARROW_peaks"
            + "/" + sub(basename(SE_all.peakbedfile), "_peaks.bed", "") else sub(basename(
            sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "+control/single-end_mode/PEAKS_Annotation/NARROW_peaks"
            + "/" + sub(basename(SE_all.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as SE_nomodel_peaksanno { input:
        gtffile = gtf,
        bedfile = SE_nomodel.peakbedfile,
        chromsizes = samtools_faidx.chromsizes,
        summitfile = SE_nomodel.summitsfile,
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS_Annotation/NARROW_peaks'
            + '/' + sub(basename(SE_nomodel.peakbedfile), '_peaks.bed', '')}" else if multi_fastqpair
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS_Annotation/NARROW_peaks"
            + "/" + sub(basename(SE_nomodel.peakbedfile), "_peaks.bed", "") else sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "+control/single-end_mode/PEAKS_Annotation/NARROW_peaks" + "/" + sub(
            basename(SE_nomodel.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as SE_sicer_peaksanno { input:
        gtffile = gtf,
        bedfile = select_first([
            SE_sicer.fdrisland,
            SE_string_ctrlwig,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/PEAKS_Annotation/BROAD_peaks'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/PEAKS_Annotation/BROAD_peaks"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/PEAKS_Annotation/BROAD_peaks",
    }

    # Motif Analysis
    if (run_motifs) {
        call motifs.motifs as SE_motifs { input:
            reference = reference,
            reference_index = samtools_faidx.faidx_file,
            bedfile = SE_macs.peakbedfile,
            motif_databases = motif_databases,
            default_location = if defined(results_name) then "~{results_name + '/single-end_mode/MOTIFS'}"
                else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/MOTIFS"
                else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "+control/single-end_mode/MOTIFS",
        }

        call util.flankbed as SE_flankbed { input:
        ...
        call motifs.motifs as SE_flank { input:
            reference = reference,
            reference_index = samtools_faidx.faidx_file,
    ...
    call viz.visualization as SE_visualization { input:
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks'
            ...
            then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/NARROW_peaks"
            + "/" + sub(basename(SE_macs.peakbedfile), "_peaks.bed", "") else sub(basename(
            sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", "") + "+control/single-end_mode/COVERAGE_files/NARROW_peaks"
            + "/" + sub(basename(SE_macs.peakbedfile), "_peaks.bed", ""),
    }

    call viz.visualization as SE_vizall { input:
    ...
    call viz.visualization as SE_viznomodel { input:
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks'
            ...
            + "/" + sub(basename(SE_nomodel.peakbedfile), "_peaks.bed", "") else sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "+control/single-end_mode/COVERAGE_files/NARROW_peaks" + "/" + sub(
            basename(SE_nomodel.peakbedfile), "_peaks.bed", ""),
    }

    call viz.visualization as SE_vizsicer { input:
        wigfile = SE_sicer.wigfile,
        chromsizes = samtools_faidx.chromsizes,
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/COVERAGE_files/BROAD_peaks'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/COVERAGE_files/BROAD_peaks"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/COVERAGE_files/BROAD_peaks",
    }
    ...
    call macs.macs as only_s_macs { input:
        bamfile = SE_s_mergebam_afterbklist,
        pvalue = "1e-9",
        keep_dup = "auto",
        egs = egs.genomesize,
        default_location = if multi_fastqpair then "SAMPLE/AllCases_" + length(
            sample_fastqfiles) + "fastqpairs/single-end_mode/PEAKS_forQC/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "-p9_kd-auto" else "SAMPLE/" + sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/PEAKS_forQC/" + basename(SE_s_mergebam_afterbklist, ".bam"
            ) + "-p9_kd-auto",
        coverage_location = if multi_fastqpair then "SAMPLE/AllCases_" + length(
            sample_fastqfiles) + "fastqpairs/single-end_mode/PEAKS_forQC/" + basename(
            SE_s_mergebam_afterbklist, ".bam") + "-p9_kd-auto" else "SAMPLE/" + sub(
            basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/PEAKS_forQC/" + basename(SE_s_mergebam_afterbklist, ".bam"
    ...
    call macs.macs as only_c_macs { input:
        bamfile = SE_c_mergebam_afterbklist,
        pvalue = "1e-9",
        keep_dup = "auto",
        egs = egs.genomesize,
        default_location = if multi_control_fastqpair then "CONTROL/AllControls_" + length(
            control_fastqfiles) + "fastqpairs/single-end_mode/PEAKS_forQC/" + basename(
            SE_c_mergebam_afterbklist, ".bam") + "-p9_kd-auto" else "CONTROL/" + sub(
            basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz", ""
            ) + "/single-end_mode/PEAKS_forQC/" + basename(SE_c_mergebam_afterbklist, ".bam"
            ) + "-p9_kd-auto",
    ...
    call bedtools.bamtobed as SE_c_finalbed { input: bamfile = SE_c_mergebam_afterbklist }

    call sortbed.sortbed as SE_c_sortbed { input: bedfile = SE_c_finalbed.bedfile }

    call bedtools.intersect as SE_c_intersect { input:
    ...
    call bedtools.bamtobed as SE_s_finalbed { input: bamfile = SE_s_mergebam_afterbklist }

    call sortbed.sortbed as SE_s_sortbed { input: bedfile = SE_s_finalbed.bedfile }

    call bedtools.intersect as SE_s_intersect { input:
    ...
    call bedtools.intersect as SE_intersect { input:
    ...
    File PE_sample_bam = select_first([
        s_PE_mergebam_afterbklist,
        s_uno_PE_mapping.bklist_bam,
        s_uno_PE_mapping.sorted_bam,
    ])
    File PE_control_bam = select_first([
        c_PE_mergebam_afterbklist,
        c_uno_PE_mapping.bklist_bam,
        c_uno_PE_mapping.sorted_bam,
    ])

    call macs.macs as PE_macs { input:
        bamfile = PE_sample_bam,
        control = PE_control_bam,
        pvalue = "1e-9",
        keep_dup = "auto",
        egs = egs.genomesize,
        output_name = if defined(results_name) then "~{results_name + '-p9_kd-auto'}" else basename(
            PE_sample_bam, ".bam") + "+control-p9_kd-auto",
        default_location = if defined(results_name) then "~{results_name + '/PEAKS/NARROW_peaks/'
            + results_name + '-p9_kd-auto'}" else sub(basename(PE_sample_bam), ".sorted.b.*$",
            "") + "+control/PEAKS/NARROW_peaks" + "/" + basename(PE_sample_bam, ".bam") + "+control-p9_kd-auto",
        coverage_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + results_name + '-p9_kd-auto'}" else sub(basename(PE_sample_bam), ".sorted.b.*$",
            "") + "+control/COVERAGE_files/NARROW_peaks" + "/" + basename(PE_sample_bam, ".bam"
            ) + "+control-p9_kd-auto",
    }

    call util.addreadme as PE_addreadme { input: default_location = if defined(
        results_name) then "~{results_name + '/PEAKS'}" else sub(basename(PE_sample_bam), ".sorted.b.*$",
        "") + "+control/PEAKS" }

    call macs.macs as PE_all { input:
        bamfile = PE_sample_bam,
        control = PE_control_bam,
        pvalue = "1e-9",
        keep_dup = "all",
        egs = egs.genomesize,
        output_name = if defined(results_name) then "~{results_name + '-p9_kd-all'}" else basename(
            PE_sample_bam, ".bam") + "+control-p9_kd-all",
        default_location = if defined(results_name) then "~{results_name + '/PEAKS/NARROW_peaks/'
            + results_name + '-p9_kd-all'}" else sub(basename(PE_sample_bam), ".sorted.b.*$",
            "") + "+control/PEAKS/NARROW_peaks" + "/" + basename(PE_sample_bam, ".bam") + "+control-p9_kd-all",
        coverage_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + results_name + '-p9_kd-all'}" else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/NARROW_peaks"
            + "/" + basename(PE_sample_bam, ".bam") + "+control-p9_kd-all",
    }

    call macs.macs as PE_nomodel { input:
        bamfile = PE_sample_bam,
        control = PE_control_bam,
        nomodel = true,
        egs = egs.genomesize,
        output_name = if defined(results_name) then "~{results_name + '-nm'}" else basename(
            PE_sample_bam, ".bam") + "+control-nm",
        default_location = if defined(results_name) then "~{results_name + '/PEAKS/NARROW_peaks/'
            + results_name + '-nm'}" else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS/NARROW_peaks"
            + "/" + basename(PE_sample_bam, ".bam") + "+control-nm",
        coverage_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + results_name + '-nm'}" else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/NARROW_peaks"
            + "/" + basename(PE_sample_bam, ".bam") + "+control-nm",
    }

    call peaseq_util.fraggraph as s_fraggraph { input:
        bamfile = select_first([
            s_PE_merge_markdup.mkdupbam,
            s_uno_PE_mapping.mkdup_bam,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        ...
        annotation_location = "SAMPLE/" + sub(basename(PE_sample_bam), ".sorted.b.*$", "")
            + "/QC/Fragments",
    }

    call samtools.indexstats as s_frag_index { input:
        bamfile = s_fraggraph.fragbamfile,
        default_location = "SAMPLE/" + sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "/BAM_files",
    }

    call peaseq_util.fraggraph as c_fraggraph { input:
        bamfile = select_first([
            c_PE_merge_markdup.mkdupbam,
            c_uno_PE_mapping.mkdup_bam,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        default_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", "")
            + "/COVERAGE_files/BAM_Fragments",
        bam_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", "") + "/BAM_files",
        annotation_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", ""
            ) + "/QC/Fragments",
    }

    call samtools.indexstats as c_frag_index { input:
        bamfile = c_fraggraph.fragbamfile,
        default_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", "")
            + "/BAM_files",
    }

    call bamtogff.bamtogff as PE_bamtogff { input:
        gtffile = gtf,
        chromsizes = samtools_faidx.chromsizes,
        bamfile = s_fraggraph.fragbamfile,
        ...
        samplename = if defined(results_name) then "~{results_name}" else basename(
            PE_sample_bam, ".bam") + "+control",
        default_location = if defined(results_name) then "~{results_name + '/BAM_Density'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/BAM_Density",
    }

    call sicer.sicer as PE_sicer { input:
        ...
        chromsizes = samtools_faidx.chromsizes,
        genome_fraction = egs.genomefraction,
        outputname = if defined(results_name) then "~{results_name}" else basename(s_fraggraph.bedpefile,
            ".bed") + "+control",
        default_location = if defined(results_name) then "~{results_name + '/PEAKS/BROAD_peaks'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS/BROAD_peaks",
        coverage_location = if defined(results_name) then "~{results_name + '/PEAKS/BROAD_peaks'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/BROAD_peaks",
    }

    call rose.rose as PE_rose { input:
        gtffile = gtf,
        bamfile = s_fraggraph.fragbamfile,
        bamindex = s_frag_index.indexbam,
        ...
        default_location = if defined(results_name) then "~{results_name + '/PEAKS/STITCHED_peaks'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS/STITCHED_peaks",
    }

    call runspp.runspp as PE_runspp { input:
        bamfile = PE_sample_bam,
        control = PE_control_bam,
    }

    call runspp.runspp as s_PE_runspp { input: bamfile = PE_sample_bam }

    call runspp.runspp as c_PE_runspp { input: bamfile = PE_control_bam }

    String string_ctrlwig = ""
    call viz.visualization as c_PE_visualization { input:
        wigfile = select_first([
            PE_macs.ctrlwigfile,
            string_ctrlwig,
        ]),
        ...
        default_location = if defined(results_name) then "~{results_name + "/COVERAGE_files/NARROW_peaks/"
            + sub(basename(PE_macs.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/NARROW_peaks/"
            + sub(basename(PE_macs.peakbedfile), "_peaks.bed", "") + "/control",
    }

    call viz.visualization as c_PE_vizall { input:
        wigfile = select_first([
            PE_all.ctrlwigfile,
            string_ctrlwig,
        ]),
        ...
        default_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + sub(basename(PE_all.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/NARROW_peaks/"
            + sub(basename(PE_all.peakbedfile), "_peaks.bed", "") + "/control",
    }
    call viz.visualization as c_PE_viznomodel { input:
        wigfile = select_first([
            PE_nomodel.ctrlwigfile,
            string_ctrlwig,
        ]),
        chromsizes = samtools_faidx.chromsizes,
        control = true,
        ...
        default_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + sub(basename(PE_nomodel.peakbedfile), '_peaks.bed', '') + '/control'}" else sub(
            basename(PE_sample_bam), '.sorted.b.*$', '') + '+control/COVERAGE_files/NARROW_peaks/'
            + sub(basename(PE_nomodel.peakbedfile), "_peaks.bed", "") + '/control',
    }

    call util.peaksanno as PE_peaksanno { input:
        gtffile = gtf,
        bedfile = PE_macs.peakbedfile,
        chromsizes = samtools_faidx.chromsizes,
        summitfile = PE_macs.summitsfile,
        default_location = if defined(results_name) then "~{results_name + '/PEAKS_Annotation/NARROW_peaks/'
            + sub(basename(PE_macs.peakbedfile), '_peaks.bed', '')}" else sub(basename(
            PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS_Annotation/NARROW_peaks/"
            + sub(basename(PE_macs.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as PE_all_peaksanno { input:
        gtffile = gtf,
        bedfile = PE_all.peakbedfile,
        chromsizes = samtools_faidx.chromsizes,
        summitfile = PE_all.summitsfile,
        default_location = if defined(results_name) then "~{results_name + '/PEAKS_Annotation/NARROW_peaks/'}"
            + sub(basename(PE_all.peakbedfile), "_peaks.bed", "") else sub(basename(
            PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS_Annotation/NARROW_peaks/"
            + sub(basename(PE_all.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as PE_nomodel_peaksanno { input:
        gtffile = gtf,
        bedfile = PE_nomodel.peakbedfile,
        chromsizes = samtools_faidx.chromsizes,
        summitfile = PE_nomodel.summitsfile,
        default_location = if defined(results_name) then "~{results_name + '/PEAKS_Annotation/NARROW_peaks/'}"
            + sub(basename(PE_nomodel.peakbedfile), "_peaks.bed", "") else sub(basename(
            PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS_Annotation/NARROW_peaks/"
            + sub(basename(PE_nomodel.peakbedfile), "_peaks.bed", ""),
    }

    call util.peaksanno as PE_sicer_peaksanno { input:
        gtffile = gtf,
        bedfile = select_first([
            PE_sicer.fdrisland,
            string_ctrlwig,
        ...
        chromsizes = samtools_faidx.chromsizes,
        default_location = if defined(results_name) then "~{results_name + '/PEAKS_Annotation/BROAD_peaks'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/PEAKS_Annotation/BROAD_peaks",
    }

    # Motif Analysis
    if (run_motifs) {
        call motifs.motifs as PE_motifs { input:
            reference = reference,
            reference_index = samtools_faidx.faidx_file,
            bedfile = PE_macs.peakbedfile,
            motif_databases = motif_databases,
            default_location = if defined(results_name) then "~{results_name + '/MOTIFS'}" else sub(
                basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/MOTIFS",
        }

        call util.flankbed as PE_flankbed { input:
            bedfile = PE_macs.summitsfile,
            default_location = if defined(results_name) then "~{results_name + '/MOTIFS'}" else sub(
                basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/MOTIFS",
        }

        call motifs.motifs as PE_flank { input:
            reference = reference,
            reference_index = samtools_faidx.faidx_file,
    ...
    call viz.visualization as PE_visualization { input:
        wigfile = PE_macs.wigfile,
        chromsizes = samtools_faidx.chromsizes,
        xlsfile = PE_macs.peakxlsfile,
        default_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/NARROW_peaks/'
            + sub(basename(PE_macs.peakbedfile), '_peaks.bed', '')}" else sub(basename(
            PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/NARROW_peaks/"
            + sub(basename(PE_macs.peakbedfile), "_peaks.bed", ""),
    }

    call viz.visualization as PE_vizall { input:
    ...
    call viz.visualization as PE_viznomodel { input:
    ...
    call viz.visualization as PE_vizsicer { input:
        wigfile = PE_sicer.wigfile,
        chromsizes = samtools_faidx.chromsizes,
        default_location = if defined(results_name) then "~{results_name + '/COVERAGE_files/BROAD_peaks'}"
            else sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "+control/COVERAGE_files/BROAD_peaks",
    }

    #Peak Calling for Sample BAM only
    call macs.macs as only_s_PE_macs { input:
        bamfile = PE_sample_bam,
        pvalue = "1e-9",
        keep_dup = "auto",
        egs = egs.genomesize,
        default_location = "SAMPLE/" + sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "/PEAKS_forQC/"
            + basename(PE_sample_bam, ".bam") + "-p9_kd-auto",
        coverage_location = "SAMPLE/" + sub(basename(PE_sample_bam), ".sorted.b.*$", "") + "/PEAKS_forQC/"
            + basename(PE_sample_bam, ".bam") + "-p9_kd-auto",
    }

    #Peak Calling for Control BAM only
    call macs.macs as only_c_PE_macs { input:
        bamfile = PE_control_bam,
        pvalue = "1e-9",
        keep_dup = "auto",
        egs = egs.genomesize,
        default_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", "")
            + "/PEAKS_forQC/" + basename(PE_control_bam, ".bam") + "-p9_kd-auto",
        coverage_location = "CONTROL/" + sub(basename(PE_control_bam), ".sorted.b.*$", "")
            + "/PEAKS_forQC/" + basename(PE_control_bam, ".bam") + "-p9_kd-auto",
    }

    call bedtools.bamtobed as only_c_PE_finalbed { input: bamfile = PE_control_bam }

    call sortbed.sortbed as only_c_PE_sortbed { input: bedfile = only_c_PE_finalbed.bedfile
        }

    call bedtools.intersect as only_c_PE_intersect { input:
    ...
    call bedtools.bamtobed as only_s_PE_finalbed { input: bamfile = PE_sample_bam }

    call sortbed.sortbed as only_s_PE_sortbed { input: bedfile = only_s_PE_finalbed.bedfile
        }

    call bedtools.intersect as only_s_PE_intersect { input:
    ...
    call bedtools.intersect as PE_intersect { input:
    ...
    File s_mergehtmlfile = select_first([
        s_final_mergehtml.mergefile,
        SE_s_mergehtml.mergefile,
    ])
    File c_mergehtmlfile = select_first([
        c_final_mergehtml.mergefile,
        SE_c_mergehtml.mergefile,
    ])
    ...
    ### ------------ A: analysis for SE mode  ----------- ###
    ### ------------------------------------------------- ###
    call util.evalstats as SE_s_summarystats { input:
        ...
        default_location = if multi_fastqpair then "SAMPLE/AllCases_" + length(
            sample_fastqfiles) + "fastqpairs/single-end_mode/QC/SummaryStats" else "SAMPLE/"
            + sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "/single-end_mode/QC/SummaryStats",
    }

    call util.evalstats as merge_SE_summarystats { input:
        bambed = SE_s_finalbed.bedfile,
    ...
    call util.evalstats as SE_c_summarystats { input:
        ...
        default_location = if multi_control_fastqpair then "CONTROL/AllControls_" + length(
            control_fastqfiles) + "fastqpairs/single-end_mode/QC/SummaryStats" else "CONTROL/"
            + sub(basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "/single-end_mode/QC/SummaryStats",
    }
    ...
    call util.concatstats as SE_concatstats {
        ...
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode/QC/SummaryStats'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode/QC/SummaryStats"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode/QC/SummaryStats",
    }
    ...
    call util.summaryreport as SE_overallsummary {
        ...
        fastq_mode = "PEAseq SEmode",
        default_location = if defined(results_name) then "~{results_name + '/single-end_mode'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/single-end_mode"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/single-end_mode",
    }
    ...
    if (one_fastqpair) {
        call util.evalstats as PE_s_summarystats { input:
            ...
            default_location = "SAMPLE/" + sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats",
        }
    ...
    if (one_control_fastqpair) {
        call util.evalstats as PE_c_summarystats { input:
            ...
            default_location = "CONTROL/" + sub(basename(control_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
                "") + "/QC/SummaryStats",
        }
    }  #if one_fastqpair
    ...
    if (multi_control_fastqpair) {
        call util.evalstats as PE_merge_c_summarystats { input:
            fastq_type = "PEAseq PEmode Control",
            bambed = only_c_PE_finalbed.bedfile,
            sppfile = c_PE_runspp.spp_out,
            fastqczip = select_first([
                c_PE_mergebamfqc.zipfile,
                string_qual,
            ]),
    ...
    call util.concatstats as PE_concatstats {
        ...
        peaseq = true,
        sample_config = select_first([
            PE_s_summarystats.configfile,
            PE_merge_s_summarystats.configfile,
        ]),
        control_config = select_first([
            PE_c_summarystats.configfile,
            PE_merge_c_summarystats.configfile,
        ]),
        overall_config = select_first([
            PE_all_summarystats.configfile,
            PE_merge_summarystats.configfile,
        ]),
        outputfile = if defined(results_name) then "~{results_name}" else "AllCases_" + length(
            sample_fastqfiles) + "fastqpairs+control",
        default_location = if defined(results_name) then "~{results_name + '/QC/SummaryStats'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control/QC/SummaryStats"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control/QC/SummaryStats",
    }
    ...
    call peaseq_util.pairedend_summaryreport as PE_overallsummary {
        ...
        sampleqc_pe_html = select_first([
            s_PE_mergehtml.xhtml,
            PE_s_summarystats.xhtml,
        ]),
        controlqc_pe_html = select_first([
            c_PE_mergehtml.xhtml,
            PE_c_summarystats.xhtml,
        ]),
        overallqc_pe_html = PE_concatstats.xhtml,
        sampleqc_pe_txt = select_first([
            s_PE_mergehtml.mergetxt,
            PE_s_summarystats.textfile,
        ]),
        controlqc_pe_txt = select_first([
            c_PE_mergehtml.mergetxt,
            PE_c_summarystats.textfile,
        ]),
        overallqc_pe_txt = PE_concatstats.textfile,
        outputfile = if defined(results_name) then "~{results_name + '.peaseq_report.html'}"
            else if multi_fastqpair then "AllCases_" + length(sample_fastqfiles) + "fastqpairs+control.peaseq_report.html"
            else sub(basename(sample_fastqfiles[0].left), "_R?[12]_....f.*q.gz|_R?[12].f.*q.gz",
            "") + "+control.peaseq_report.html",
    }
