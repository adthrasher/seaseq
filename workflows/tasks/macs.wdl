...
task macs {
    input {
        ...
        String default_location = if (nomodel) then prefix_location + "/" + basename(
            bamfile, ".bam") + "-nm" else prefix_location + "/" + basename(bamfile, ".bam"
            ) + "-p" + sub(pvalue, "^.*-", "") + "_kd-" + keep_dup
        String output_name = if (nomodel) then basename(bamfile, ".bam") + "_nm"
            else basename(bamfile, ".bam") + "_p"
            + sub(pvalue, "^.*-", "") + "_kd-"
            + keep_dup
        Boolean wiggle = true
    ...
    command <<<
        ...
        mv "~{output_name}_MACS_wiggle/treat/~{
            output_name}_treat_afterfiting_all.wig.gz" \
            "~{coverage_location}"
        ...
        if [ -f "~{control}" ]; then
            mv "~{default_location}/~{output_name}_MACS_wiggle/control/~{
            output_name}_control_afterfiting_all.wig.gz" \
    ...
    output {
        ...
        File? ctrlwigfile =
            "~{coverage_location}/control/~{output_name}_control_afterfiting_all.wig.gz"
    }
