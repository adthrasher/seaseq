...
task macs {
    input {
        ...
        String output_name = if (nomodel) then basename(bamfile, ".bam") + "_nm" else basename(
            bamfile, ".bam") + "_p" + sub(pvalue, "^.*-", "") + "_kd-" + keep_dup
    }
    ...
    command <<<
        ...
        mv ~{output_name}_MACS_wiggle/treat/~{output_name}_treat_afterfiting_all.wig.gz ~{
            coverage_location}
