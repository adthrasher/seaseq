...
workflow mapping {
    ...
    if (!defined(fastqfile_R2)) {
        call bowtie.bowtie as SE_map { input:
            fastqfile = fastqfile,
    ...
    if (defined(fastqfile_R2)) {
        call bowtie.bowtie as PE_map { input:
            fastqfile = fastqfile,
    ...
    call samtools.viewsort { input:
        samfile = select_first([
            SE_map.samfile,
            PE_map.samfile,
        ]),
    ...
    call samtools.indexstats { input:
    ...
    if (defined(blacklist)) {
        ...
        if (defined(metricsfile)) {
            call bedtools.intersect as rmblklist { input:
        ...
        if (defined(fastqfile_R2)) {
            ...
            call bedtools.pairtobed as pairtobed { input:
                fileA = fixmate_file,
                fileB = blacklist_file,
                default_location = default_location,
            }
        ...
        File bklist_bamfile = select_first([
            rmblklist.intersect_out,
            pairtobed.pairtobed_out,
        ])

        call samtools.indexstats as bklist { input:
    ...
    call samtools.indexstats as mkdup { input:
        bamfile = markdup.mkdupbam,
        default_location = default_location,
    }

    File parse_sorted_bam = select_first([
        viewsort.fixmatebam,
        viewsort.sortedbam,
    ])
