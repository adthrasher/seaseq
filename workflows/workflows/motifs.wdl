...
workflow motifs {
    ...
    call bedtools.bedfasta { input:
        reference = reference,
        reference_index = reference_index,
        bedfile = bedfile,
    }

    call process_motif_folder { input: inputfile = bedfasta.fastafile }

    if (defined(motif_databases)) {
        Array[String] string_motif_databases = [
            "1",
        ]
        Array[File] motif_database_files = select_first([
            motif_databases,
            string_motif_databases,
        ])
        ...
        call ame { input:
            ...
            default_location = default_location + "/" + basename(select_first(
                process_motif_folder.placeholder_output)) + "-ame_out",
        }
    }
    call meme { input:
        fastafile = bedfasta.fastafile,
